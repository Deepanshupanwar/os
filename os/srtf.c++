...
int main()
{
    ...
    while(completed!=n)
    {
        ...
        if(idx!=-1)
        {
            if(burst_remaining[idx]==arr[idx].burst_time)
            {
                ...
                total_idle_time+=arr[idx].start_time-prev;
            }
            current_time++;
            prev=current_time;
            burst_remaining[idx]--;
    ...
    int max_completed_time=-1;
    int min_arrival_time=1000000;
    for(int i=0;i<n;i++)
    {
        max_completed_time=max(max_completed_time,arr[i].completion_time);
        min_arrival_time=min(min_arrival_time,arr[i].arrival_time);
    }
    avg_turnaround_time = (float) total_turnaround_time / n;
    avg_waiting_time = (float) total_waiting_time / n;
    avg_response_time = (float) total_response_time / n;
    cpu_utilisation = ((max_completed_time - total_idle_time) / (float) max_completed_time )*100;
